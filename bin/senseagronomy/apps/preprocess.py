"""
This module applies linear transformations and masks out erroneous pixels based on quality flags.
Additionally, the output can be converted to integer values and scaled accordingly to save disk
space. It's expected that an input directory is given with a stacked image containing the surface
reflectance bands, metadata file in XML format as well as quality images for pixel, radiometric,
and aerosol processing.
"""
...
def main() -> int:
    ...
    parser = ArgumentParser(
        ...
        description=(
            "This program is used to apply linear transformations and mask out erroneous "
            "pixels based on quality flags. Additionally, the output can be converted to "
            "integer values and scaled accordingly to save disk space. It's expected that "
            "an input directory is given with a stacked image containing the surface "
            "reflectance bands, metadata file in XML format as well as quality images for "
            "pixel, radiometric, and aerosol processing."
        ),
    )
    parser.add_argument(
        "--platform",
    ...
    parser.add_argument(
        ...
        help=(
            "Pixel quality mask flags to apply. Flags prefixed with 'C' correspond to "
            "cloud confidence, flags prefixed with 'CS' correspond to cloud shadows, "
            "flags prefixed with 'SC' correspond to snow/ice confidence and flags "
            "prefixed with 'CC' correspond to cirrus confidence (not applicable when "
    ...
    parser.add_argument(
        ...
        help=(
            "Radiometric correction quality mask flags. Individual bands correspond "
    ...
    parser.add_argument(
        ...
        default=["FILL"],
        help=(
            "Aerosol processing quality mask flags. These only apply for OLI platform and "
    ...
    parser.add_argument(
        ...
        help=(
            "Aerosol processing quality mask flags. These only apply for TM platform and "
    ...
    parser.add_argument(
        ...
        required=False,
        help=(
            "Output data type. Note that choosing 'int32' will raise a runtime warning which "
    ...
    parser.add_argument(
        ...
        required=False,
        help=(
            "Scale factor for output band stack. Only applicable if 'otype' == int32. "
    ...
    radsat_mask = scene.get_radsat_qa(str2radsat(args.radsat_qa))[np.newaxis, ...]
    ...
    with rio.open(f"{args.output_dir}/{args.output}", "w", **scene.metadata) as dataset:
